...
2. `airflow-init` pide más de 4 Gb de ram, por lo que se recomienda aumentar los recursos de docker.
3. Clona este repositorio.
4. Crea las carpetas `airflow/config`, `airflow/dags`, `airflow/logs`, `airflow/plugins`, `airflow/logs`.
5. Si estás en Linux o MacOS, en el archivo `.env`, reemplaza `AIRFLOW_UID` por el de tu usuario o alguno que consideres oportuno (para encontrar el UID, usa el comando `id -u <username>`). De lo contrario, Airflow dejará sus carpetas internas como root y no podrás subir DAGs (en `airflow/dags`) o plugins, etc.
6. En la carpeta raíz de este repositorio, ejecuta:
...
![Sistema MLops.png](Sistema_MLops.png)
...
Para poder usar el modelo, es necesario hacer un registro inicial en MLFlow. Para esto, se debe ejecutar la DAG `hyperparameter_tuning`.
